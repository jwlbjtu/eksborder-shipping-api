...
export const validateCarrier = (carrier?: string, provider?: string) => {
    ...
    if(carrier.toLowerCase() === CARRIERS.PITNEY_BOWES && provider && !SUPPORTED_PROVIDERS[carrier.toLowerCase()].includes(provider)) {
        throw LRes.fieldErr("provider", "/", errorTypes.UNSUPPORTED, provider, carrier);
    }
    return { carrier, provider };
}

export const validateService = (carrier: string, provider?: string, service?: string): string => {
...
export const validateParcelType = (carrier: string, service: string, parcelType?: string) => {
    if(carrier.toLowerCase() === CARRIERS.DHL_ECOMMERCE) return;
    if(!parcelType) throw LRes.fieldErr("parcelType", "/packageDetail/parcelType", errorTypes.MISSING, carrier);
    if(!SUPPORTED_PARCEL_TYPES[service].includes(parcelType)) {
        throw LRes.fieldErr("parcelType", "/packageDetail/parcelType", errorTypes.UNSUPPORTED, parcelType);
    }
    return parcelType;
}

export const validateCarrierAccount = async (carrierAccount: string | undefined, user: IUser): Promise<{carrierAccount: string, account: IAccount}> => {
