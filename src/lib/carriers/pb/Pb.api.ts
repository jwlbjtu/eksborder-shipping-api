...
class PbApi implements ICarrierAPI {
  ...
  public rules: any = async (
    ...
    try {
      ...
      const response = await AxiosapiLib.doCall(
        'get',
        this.api_url +
          `/shippingservices/v1/information/rules/rating-services?carrier=${carrier}&originCountryCode=${originCountryCode}&destinationCountryCode=${destinationCountryCode}`,
        null,
        headers
      );
      return response;
  ...
  public products: any = async (data: IProductRequest) => {
    ...
    try {
      const response = await AxiosapiLib.doCall(
        'post',
        this.api_url + '/shippingservices/v1/rates?' + paramsStr,
        prodReqBody,
        headers
      );
  ...
  public label: any = async (data: ILabelRequest) => {
    ...
    try {
      const response = await AxiosapiLib.doCall(
        'post',
        this.api_url + '/shippingservices/v1/shipments',
        labelReqBody,
        headers
      );
      await saveLog(
  ...
  public manifest: any = async (data: IManifestRequest) => {
    ...
    try {
      ...
      const response = await AxiosapiLib.doCall(
        'post',
        this.api_url + '/shippingservices/v1/manifests',
        manifestBody,
        headers
      );
      await saveLog(
  ...
  private convertToProducts(data: IPBRatesResponse, currency: string) {
    ...
    const products = rates.map((rate: IPBRate) => {
      ...
      if (rate.surcharges) {
        const surcharges = rate.surcharges.map(
          (charge: { name: string; fee: number }) => {
      ...
      if (rate.deliveryCommitment) {
        product.estimatedDeliveryDate = {
          ...
          deliveryDaysMax: parseInt(
            rate.deliveryCommitment.maxEstimatedNumberOfDays
          ),
          deliveryDaysMin: parseInt(
            rate.deliveryCommitment.minEstimatedNumberOfDays
          ),
