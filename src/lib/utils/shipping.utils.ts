...
import { IProductRequest, IProduct, IProductResponse } from "../../types/shipping.types";
...
class ShippingUtil {
    static initCF = async (account: IAccount, user: IUser): Promise<ICarrierAPI> => {
        // @ts-ignore
        const api = CarrierFactory.getCarrierAPI(account.carrierRef.carrierName, {user, account});
        await api.auth();
        return api;
    };
    ...
    static getProducts = async (account: IAccount, user: IUser, productRequest: IProductRequest, weight: number, carrier: string, service?: string) => {
        let callAPI: boolean = false;
        let getFlat: boolean = false;
        ...
        if(getFlat) {
            if(weight > 16) return lresponseLib.getError(500, "FLAT Mail cannot more than 16 oz");
