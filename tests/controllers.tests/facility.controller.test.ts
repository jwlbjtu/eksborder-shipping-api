    ...
    it("Should create facility for admin user", async () => {
        await request(app)
            ...
            .expect(async (res) => {
                ...
                const carrier = await Carrier.findById(dhlCarrier._id).populate({path: 'facilityRef'});
                // @ts-ignore
                expect(carrier.facilityRef.length).toEqual(2);
                // @ts-ignore
                expect(carrier.facilityRef[1].facilityNumber).toEqual(dhlFacility.facilityNumber);
            });
    });
